...
def scrape_details():
    """Carga las URLs y extrae los detalles de cada propiedad, dividiendo los resultados según el tipo."""
    try:
        # Cargar las URLs del archivo generado en el bloque 1
        df_urls = pd.read_csv("urls_propiedades_paginas.csv")
        df_urls = pd.concat([df_urls.head(5), df_urls.tail(5)], ignore_index=True)
        ...
        # Dividir el DataFrame según el tipo de inmueble
        df_venta = df_details[df_details["Tipo"] == "Venta"]
        df_arrendamiento = df_details[df_details["Tipo"] == "Arrendamiento"]

        # Guardar los resultados en archivos separados
        if not df_venta.empty:
            df_venta.to_csv("detalles_venta.csv", index=False, encoding="utf-8-sig")
            print("Datos de venta guardados en detalles_venta.csv")
        if not df_arrendamiento.empty:
            df_arrendamiento.to_csv("detalles_arrendamiento.csv", index=False, encoding="utf-8-sig")
            print("Datos de arrendamiento guardados en detalles_arrendamiento.csv")
    except Exception as e:
